...
rearrange_output_data = function(df, readsim_cov = 100000, id_list, chromlist){
    ...
    suppressWarnings({
      # warning messages will occur for samples that do not have second parameter

      # warnings will pop up bc some want have all categories to sep into. this is okay
    ...
    suppressWarnings({
      # separate funct will cause warnings 
...
adjust_counts_syn = function(count_df){
...
adjust_information_types_syn = function(df){
    df$total_input_snps = df$TP + df$FN
    df$total_positives = df$FP + df$TP
    df$total_snps = df$FP + df$TP + df$FN
    df$tp_ratio = df$TP/df$total_snps
