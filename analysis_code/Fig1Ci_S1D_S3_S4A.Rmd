...
- Supp. Fig1D
- Supp. FigS3A-B
Othere data generated in this notebook was not used in the final version of the manuscript
...
## What is the relationship between observed and expected allele frequency estimates? Fig 1C(influenza) & Fig S3B
```{r}
...
Fig1Ci = ggplot(TP %>% filter(!FerretID %in% c("Mix37", "Mix38", "Mix39", "Mix40", "Mix41", "Mix42", "Mix44")), aes(x = factor(round(medianfreq, digits = 3)), y = ALT_FREQ, color = vcf_inputs))+
...
FigS3B = ggplot(TP, aes(x = as.numeric(Expected_freq), y = ALT_FREQ, color = Copy_number))+
...
print(FigS3B)
ggsave(FigS3B,
       filename = glue("{workdirectory}/figures_new/FigS4A.png"),
       width = 16,
       height = 6.5, limitsize=FALSE)#, useDingbats = FALSE)
ggsave(FigS3B,
       filename = glue("{workdirectory}/figures_pdf_new/FigS4A.pdf"),
       width = 16,
       height = 6.5, limitsize=FALSE)#, useDingbats = FALSE)
```

## Coverage plots - Fig S3A
```{r}
...
FigS3A = ggplot(data = flu_cov, aes(x=ntpos, y=log10(totalcount), color = Sample)) +
...
print(FigS3A)
ggsave(FigS3A,
       filename = glue("{workdirectory}/FigS3.png"),
       width = 12,
       height = 7, limitsize=FALSE)#, useDingbats = FALSE)
ggsave(FigS3A,
       filename = glue("{workdirectory}/figures_pdf/FigS3.pdf"),
       width = 12,
       height = 7, limitsize=FALSE)#, useDingbats = FALSE)
...
## How we find the transcription errors
```{r}
...
## what's the relationship between allele freq estimates between replicates?
```{r}
...
## SNV map: showing both engineered SNVs and in-vitro transcription errors - FigureS1D
```{r}
...
print(FigS1D)
ggsave(FigS1D,
       filename = glue("{workdirectory}/Figures/FigS1D.png"),
       width = 17,
       height = 2, limitsize=FALSE)#, useDingbats = FALSE)
ggsave(FigS1D,
       filename = glue("{workdirectory}/figures_pdf/FigS1D.pdf"),
       width = 17,
       height = 2, limitsize=FALSE)#, useDingbats = FALSE)
...
## How well the variant callers agreed on the frequency of the variants predicted? 
```{r}
...
## How does having cutoffs affect variant calling?
## These versions were not used in paper.
```{r}
#Construct cutoff df
...
## Where are the FPs - not used in paper
```{r}
